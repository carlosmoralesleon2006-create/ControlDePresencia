...
from models import Empleado, Rol, Horario, FranjaHoraria, Empresa, Registro, Incidencia
...
@login_required
def registro():
    if current_user.rol_obj.nombre not in ['Administrador', 'Superadministrador']:
    ...
    if request.method == 'POST':
        ...
        if not id_empresa:
            flash('Debes seleccionar o tener una empresa asignada.', 'danger')
...
def empresa_nuevo():
    ...
    # Capturar nuevos campos del mapa
    lat = request.form.get('lat', type=float)
    lng = request.form.get('lng', type=float)
    radio = request.form.get('radio', type=float, default=50.0)
...
def empresa_modificar(empresa_id):
    ...
    try:
        ...
        # Actualizar campos de geolocalización desde el mapa
...
@main_bp.route('/gestion_empleados')
@login_required
def gestion_empleados():
    ...
    if current_user.rol_obj.nombre == 'Superadministrador':
        empleados = Empleado.query.filter_by(id_rol=rol_empleado.id).all()
        ...
        empresas_disponibles = [current_user.empresa_obj]
...
def ver_incidencias():
    ...
    if current_user.rol_obj.nombre == 'Superadministrador':
        ...
        incidencias = Incidencia.query.join(Empleado).filter(
            Empleado.id_empresa == current_user.id_empresa
        ).order_by(Incidencia.fecha_hora.desc()).all()

    return render_template('ver_incidencias.html', incidencias=incidencias)